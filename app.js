const $=s=>document.querySelector(s), cfg=window.BOARD_CONFIG||{}, live=!!(cfg.supabaseUrl&&cfg.supabaseKey), key='common-ground-v1';
let saved;try{saved=JSON.parse(localStorage.getItem(key))||{};}catch{saved={};}
let identity=saved.identity||{id:crypto.randomUUID(),name:''},user={id:identity.id},profile=null,db,ideas=[],votes=[],comments=[],opened=new Set();
function save(){localStorage.setItem(key,JSON.stringify({identity,ideas,votes,comments}));}
...
async function busy(b,fn){b.disabled=true;try{await fn();}catch(e){toast('Could not save. '+e.message);}finally{b.disabled=false;}}
function needName(){if(profile)return true;$('#login').showModal();return false;}
...
$('#body').oninput=()=>{$('#count').textContent=`${$('#body').value.length.toLocaleString()} / 4,000`;$('#body').setCustomValidity('');};$('#title').oninput=()=>$('#title').setCustomValidity('');
$('#idea-form').onsubmit=e=>{e.preventDefault();if(!needName())return;const title=$('#title').value.trim(),body=$('#body').value.trim();if(!title||!body){const input=$(title?'#body':'#title');input.setCustomValidity('Please add some text.');input.reportValidity();return;}busy(e.submitter,async()=>{await insert('ideas',{user_id:user.id,author_name:profile.name,title,body});$('#idea-form').reset();$('#count').textContent='0 / 4,000';await refresh();toast('Your idea is on the table.');});};$('#sort').onchange=render;
async function start(){$('#mode').textContent=live?'Shared group board · Enter your name to post, vote, and comment.':'Local preview · Posts, votes, and comments stay in this browser until the shared board is connected.';if(live){const {createClient}=await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm');db=createClient(cfg.supabaseUrl,cfg.supabaseKey);user=check(await db.auth.getSession()).session?.user;if(user)profile=check(await db.from('profiles').select('name').eq('id',user.id).maybeSingle());await refresh();}else{ideas=saved.ideas||[];votes=saved.votes||[];comments=saved.comments||[];profile=identity.name?{name:identity.name}:null;render();}showIdentity();if(!profile)$('#login').showModal();}
start().catch(e=>{$('#mode').textContent='The shared board could not connect. Reload to try again.';toast(e.message);});
setInterval(()=>{if(live&&db&&document.visibilityState==='visible'&&!$('#login').open&&!opened.size&&!document.activeElement.matches('input,textarea,button'))refresh().catch(()=>{});},30000);
